...
if (isset($_GET['itemId'])) {
    ...
    if (isset($_POST['userId'], $_POST['content'])) {
        // POST variables exist, insert a new comment into the MySQL comments table (user submitted form)
        $stmt = $pdo->prepare('INSERT INTO comments (groceryItemId, parent_id, name, content, submit_date) VALUES (?,?,?,?,NOW())');
        $stmt->execute([ $_GET['itemId'], $_POST['parent_id'], $_POST['userId'], $_POST['content'] ]);
        exit('Your comment has been submitted!');
    }
    // Get all comments by the Page ID ordered by the submit date
    $stmt = $pdo->prepare('SELECT * FROM comments WHERE groceryItemId = ? ORDER BY submit_date DESC');
    $stmt->execute([ $_GET['itemId'] ]);
    $comments = $stmt->fetchAll(PDO::FETCH_ASSOC);
    // Get the total number of comments
    $stmt = $pdo->prepare('SELECT COUNT(*) AS total_comments FROM comments WHERE groceryItemId = ?');
    $stmt->execute([ $_GET['itemId'] ]);
    $comments_info = $stmt->fetch(PDO::FETCH_ASSOC);
} else {
    exit('No item ID specified!');
}
